...
class ASP_Recaptcha {
    ...
    public function enqueueRecaptchaScripts() {
        ...
        if ($this->v3_enabled) {
            wp_enqueue_script('recaptcha-v3', 'https://www.google.com/recaptcha/api.js?render=' . $this->site_key, array(), null, true);
            ...
            grecaptcha.ready(function() {
                ...
                if (loginForm) {
                    loginForm.addEventListener('submit', function(e) {
                        e.preventDefault();
                        grecaptcha.execute('" . $this->site_key . "', {action: 'login'}).then(function(token) {
                ...
                if (registerForm) {
                    registerForm.addEventListener('submit', function(e) {
                        e.preventDefault();
                        grecaptcha.execute('" . $this->site_key . "', {action: 'register'}).then(function(token) {
                            var input = document.createElement('input');
                            input.type = 'hidden';
                            input.name = 'g-recaptcha-response';
                            input.value = token;
                ...
                if (lostpasswordForm) {
                    lostpasswordForm.addEventListener('submit', function(e) {
                        e.preventDefault();
                        grecaptcha.execute('" . $this->site_key . "', {action: 'lostpassword'}).then(function(token) {
                            var input = document.createElement('input');
                            input.type = 'hidden';
                            input.name = 'g-recaptcha-response';
                            input.value = token;
                ...
                if (commentForm) {
                    commentForm.addEventListener('submit', function(e) {
                        e.preventDefault();
                        grecaptcha.execute('" . $this->site_key . "', {action: 'comment'}).then(function(token) {
                            var input = document.createElement('input');
                            input.type = 'hidden';
                            input.name = 'g-recaptcha-response';
                            input.value = token;
    ...
    public function verifyLostPasswordRecaptcha() {
        if (!$this->verifyRecaptcha()) {
            wp_die(__('reCAPTCHA verification failed. Please try again.', 'advanced-security-lite'));
        }
    }
